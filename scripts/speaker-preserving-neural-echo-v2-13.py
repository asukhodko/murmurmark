#!/usr/bin/env python3
"""Select v2.13 audio after bounded iterative pre-ASR outcome rollback."""
...
POLICY_PATH = ROOT / "policies/speaker-preserving-neural-echo-v2-13.json"
WHISPER_MODEL = (
    Path.home() / ".local/share/murmurmark/models/whisper.cpp/ggml-large-v3-q5_0.bin"
)
OUTPUT_NAME = "speaker-preserving-neural-echo-v2-13"
ASR_CHUNK_CONTEXT_SEC = 5.0
...
PARENT = load_module(
    ...
    "murmurmark_spne_v213_selector_parent",
)
AUDIO = load_module(
    ROOT / "scripts/speaker-preserving-neural-echo-v2-13-audio.py",
    "murmurmark_spne_v213_audio_runtime",
)
SHADOW = load_module(
    ROOT / "scripts/speaker-preserving-echo-full-shadow-v2-13.py",
    "murmurmark_spne_v213_shadow_runtime",
)
V28 = PARENT.V28
...
def verify_policy(path: Path) -> dict[str, Any]:
    ...
    if policy.get("schema") != "murmurmark.speaker_preserving_neural_echo_policy/v2.13":
        raise RuntimeError("unexpected v2.13 policy schema")
    pairs = (
        ...
        ("v2_12_hard_decision", "v2_12_hard_decision_sha256"),
        ("v2_12_hard_report", "v2_12_hard_report_sha256"),
    )
    checks = {
    ...
    checks["hard_set_frozen"] = (
        hard_set.get("status") == "frozen_before_v2_13_implementation"
        and hard_set.get("training_use") == "forbidden"
        and hard_set.get("threshold_tuning_use") == "forbidden"
    )
    checks["corpus_set_frozen"] = (
        corpus_set.get("status") == "frozen_before_v2_13_implementation"
    ...
    checks["v2_12_rejected"] = (
        V28.read_json(policy_artifact(policy, "v2_12_hard_decision")).get("decision")
        == "HARD_TEST_REJECTED_V2_12"
    )
    if not all(checks.values()):
        raise RuntimeError(f"v2.13 policy verification failed: {checks}")
    return policy
...
def recovered_intervals(path: Path) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for parent in read_jsonl(path):
        for child in parent.get("children", []):
            if not isinstance(child, dict):
                continue
            start = float(child.get("start_ms") or 0.0) / 1000.0
            end = float(child.get("end_ms") or 0.0) / 1000.0
            if end > start:
                rows.append(
                    {
                        "start": start,
...
def lost_recovered_local_intervals(
    ...
    for row in baseline:
        ...
        best = max(
            (
                interval_match(interval, (other["start"], other["end"]))
                for other in candidate
            ),
            default=0.0,
        )
        if best < 0.5:
            lost.append({**row, "best_candidate_overlap_ratio": round(best, 6)})
    return lost
...
def outcome_rollback_chunks(
    ...
    if "local_recall_preserved" in failed:
        for row in lost_recovered_local_intervals(session, stage):
            evidence.append({"type": "lost_recovered_local_island", **row})
    ...
    decisions = read_jsonl(output / "diagnostic_chunk_decisions.jsonl")
        ...
        matching = sorted(
            {
                ...
                and min(
                    ...
                    float(row.get("hard_end_sec") or 0.0)
                    + ASR_CHUNK_CONTEXT_SEC,
                )
                > max(
                    start,
                    float(row.get("hard_start_sec") or 0.0)
                    - ASR_CHUNK_CONTEXT_SEC,
                )
            }
        )
        item["diagnostic_chunks"] = matching
        rollback.update(matching)
    return sorted(rollback), [row for row in evidence if row["diagnostic_chunks"]]
...
def archive_initial_attempt(
    ...
    payload = {
        "schema": "murmurmark.speaker_preserving_neural_echo_audio_rollback/v2.13",
        "reason": "outcome_regression_mapped_to_changed_pre_asr_chunks",
...
def archive_rollback_iteration(
    ...
    *,
    ...
    payload = {
        "schema": "murmurmark.speaker_preserving_neural_echo_rollback_iteration/v2.13",
        "pass": pass_index,
        "new_excluded_diagnostic_chunks": new_chunks,
        "cumulative_excluded_diagnostic_chunks": cumulative_chunks,
        "outcome_regression_evidence": evidence,
...
def continue_iterative_rollback(
    ...
    """Repeat safe chunk rollback until outcome gates pass or evidence runs out."""
    ...
    cumulative = {
        int(value)
        for value in initial.get("excluded_diagnostic_chunks", [])
        ...
        iterations.append(
            {
                "schema": "murmurmark.speaker_preserving_neural_echo_rollback_iteration/v2.13",
                "pass": 1,
                "new_excluded_diagnostic_chunks": sorted(cumulative),
                "cumulative_excluded_diagnostic_chunks": sorted(cumulative),
    ...
    while not all(checks.values()) and len(iterations) < max_passes:
        rollback_chunks, evidence = outcome_rollback_chunks(session, source, shadow)
        new_chunks = sorted(set(rollback_chunks) - cumulative)
        if not new_chunks:
            break
        cumulative.update(new_chunks)
        ...
        iterations.append(
            archive_rollback_iteration(
                ...
                cumulative_chunks=sorted(cumulative),
                evidence=evidence,
            )
        )
    ...
    rollback = {
        "schema": "murmurmark.speaker_preserving_neural_echo_audio_rollback/v2.13",
        "strategy": "bounded_iterative_pre_asr_chunk_rollback",
        "max_passes": max_passes,
        "passes_completed": len(iterations),
        "excluded_diagnostic_chunks": sorted(cumulative),
...
def configure_parent() -> None:
    ...
    PARENT.chronology_rollback_chunks = outcome_rollback_chunks
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    payload["schema"] = "murmurmark.speaker_preserving_neural_echo_selection/v2.13"
    payload["selection_contract"] = (
        "window_identical_exact_local_audio_then_bounded_iterative_pre_asr_outcome_rollback"
    )
...
def verify(args: argparse.Namespace) -> dict[str, Any]:
    ...
    checks = {
        ...
        == "murmurmark.speaker_preserving_neural_echo_selection/v2.13",
    ...
    payload = {
        "schema": "murmurmark.speaker_preserving_neural_echo_verification/v2.13",
