#!/usr/bin/env python3
"""Freeze and evaluate Speaker-Preserving Neural Echo v2.4 on ordinary meetings."""
...
POLICY_PATH = ROOT / "policies/speaker-preserving-neural-echo-corpus-v2-4.json"
OUTPUT = ROOT / "sessions/_reports/speaker-preserving-neural-echo-v2-4-corpus"
WHISPER_MODEL = (
...
PROMOTION = load_module(
    ...
    "murmurmark_spne_v24_corpus_shadow",
)
...
def verify_policy(path: Path) -> dict[str, Any]:
    ...
    if policy.get("schema") != "murmurmark.speaker_preserving_neural_echo_corpus_policy/v2.4":
...
def command_freeze(args: argparse.Namespace) -> int:
    ...
    payload = {
        "schema": "murmurmark.speaker_preserving_neural_echo_frozen_corpus/v2.4",
...
def verify_frozen(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if frozen.get("schema") != "murmurmark.speaker_preserving_neural_echo_frozen_corpus/v2.4":
...
def prepare_shadow_root(session: Path, candidate_audio: Path) -> Path:
    ...
    output_root = (
        session
        / "derived/preprocess/speaker-preserving-neural-echo-v2-4/corpus-evaluation"
    )
...
def run_one(
    ...
    if status not in {"candidate", "fallback"}:
        raise RuntimeError(f"{session_id}: non-terminal runtime status {status}")
    candidate_audio = (
        session
    ...
    fallback_exact = status == "fallback" and sha256(candidate_audio) == sha256(baseline_audio)
    if status == "candidate":
    ...
    payload = {
        "schema": "murmurmark.speaker_preserving_neural_echo_corpus_session/v2.4",
        "session": session_id,
...
def render_report(payload: dict[str, Any]) -> str:
    ...
    lines = [
        "# Speaker-Preserving Neural Echo v2.4 Corpus",
        "",
...
def command_report(args: argparse.Namespace) -> int:
    ...
    for session_id in ids:
        ...
        if payload.get("schema") != "murmurmark.speaker_preserving_neural_echo_corpus_session/v2.4":
    ...
    payload = {
        "schema": "murmurmark.speaker_preserving_neural_echo_corpus_report/v2.4",
        "frozen_fingerprint": frozen["fingerprint"],
        "aggregate": aggregate,
        "sessions": rows,
        "promotion": {
...
def command_verify(args: argparse.Namespace) -> int:
    ...
    payload = {
        "schema": "murmurmark.speaker_preserving_neural_echo_corpus_verification/v2.4",
