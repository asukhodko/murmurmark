#!/usr/bin/env python3
"""Build and evaluate the ASR-audited Target-Me echo suppressor v2.4."""
...
POLICY_PATH = ROOT / "policies/speaker-preserving-neural-echo-v2-4.json"
...
TARGET_ME = load_module(ROOT / "scripts/audit-target-me.py", "murmurmark_spne_v24_target_me")
TRANSCRIBER = load_module(
    ROOT / "scripts/transcribe-simple-whispercpp.py", "murmurmark_spne_v24_transcriber"
)
METRICS = load_module(
    ROOT / "scripts/report-speaker-preserving-neural-echo-v2-3-corpus.py",
    "murmurmark_spne_v24_metrics",
)
...
def verify_policy(path: Path) -> dict[str, Any]:
    ...
    if policy.get("schema") != "murmurmark.speaker_preserving_neural_echo_policy/v2.4":
        raise RuntimeError("unexpected v2.4 policy schema")
    ...
    if not all(bool(row.get("passed")) for row in checks.values()):
        raise RuntimeError(f"v2.4 policy verification failed: {checks}")
...
def enrollment_vectors(args: argparse.Namespace) -> tuple[np.ndarray, np.ndarray, dict[str, Any]]:
    manifest = verify_enrollment_manifest(args.output / "enrollment", sha256(args.policy))
    if not manifest:
        raise RuntimeError("build and verify the controlled enrollment first")
    wavlm = np.load(args.output / "enrollment/wavlm_centroid.npy", allow_pickle=False)
    resemblyzer = np.load(
        args.output / "enrollment/resemblyzer_centroid.npy", allow_pickle=False
    )
...
def session_paths(session: Path) -> dict[str, Path]:
    ...
    return {
        ...
        "reviewed_dialogue": transcript / "resolved/clean_dialogue.reviewed_v1.json",
    }
...
def proposal_basis(
    *,
    ...
    return {
        "schema": "murmurmark.speaker_preserving_neural_echo_proposal_basis/v2.4",
...
def cached_proposals(
    *,
    ...
    write_json(
        manifest_path,
        {
            "schema": "murmurmark.speaker_preserving_neural_echo_proposal_manifest/v2.4",
            "basis": basis,
...
def transcribe_clip_cached(
    *,
    ...
    seek = int(round(spec["seek_ms"] * SAMPLE_RATE / 1000.0))
    end = int(round(spec["clip_end_ms"] * SAMPLE_RATE / 1000.0))
    ...
    if force or read_json(metadata).get("basis") != basis or not output_base.with_suffix(".json").is_file():
        ...
        write_json(metadata, {"schema": "murmurmark.spne_v24_chunk_asr_cache/v1", "basis": basis})
...
def diagnostic_metrics(
    *,
    ...
    before = METRICS.remote_like(baseline, remote, states)
    after = METRICS.remote_like(candidate, remote, states)
    relevant_dialogue = {
        "utterances": [
            row
...
def diagnostic_select(
    *,
    ...
    workspace = output / "diagnostic-work"
    ...
    for index, rows in sorted(by_index.items()):
        ...
        def evaluate(attenuation: float) -> dict[str, Any]:
            ...
            write_pcm16(candidate_path, candidate)
        ...
        if remote_screen_passed:
            for attenuation in sorted((value for value in levels if value != strongest), reverse=True):
                attempts.append(evaluate(attenuation))
        ...
        if accepted:
            selected.extend(dict(row, attenuation_db=chosen, diagnostic_chunk=index) for row in rows)
...
def transcribe_final(
    *,
    ...
    payload["params"]["murmurmark_echo_profile"] = "speaker_preserving_neural_echo_v2_4"
    ...
    write_json(
        ...
        {
            "schema": "murmurmark.speaker_preserving_neural_echo_chunk_asr/v2.4",
...
def fallback(
    *, session: Path, output: Path, reason: str, basis: dict[str, Any], details: Any = None
    ...
    payload = {
        "schema": "murmurmark.speaker_preserving_neural_echo_runtime/v2.4",
        "status": "fallback",
        "reason": reason,
        "details": details,
        "basis": basis,
...
def run_session(args: argparse.Namespace) -> dict[str, Any]:
    ...
    output = session / "derived/preprocess/speaker-preserving-neural-echo-v2-4"
    ...
    basis = {
        ...
        "enrollment_manifest_sha256": sha256(args.output / "enrollment/enrollment_manifest.json"),
    ...
    try:
        ...
        shutil.rmtree(output / "diagnostic-work", ignore_errors=True)
...
def verify_session(args: argparse.Namespace) -> dict[str, Any]:
    ...
    checks = {
        ...
        == "murmurmark.speaker_preserving_neural_echo_runtime/v2.4",
    ...
    payload = {
        "schema": "murmurmark.speaker_preserving_neural_echo_verification/v2.4",
