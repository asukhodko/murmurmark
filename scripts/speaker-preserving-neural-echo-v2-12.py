#!/usr/bin/env python3
"""Select outcome-preserving v2.12 audio or the bit-exact local-FIR baseline."""
...
POLICY_PATH = ROOT / "policies/speaker-preserving-neural-echo-v2-12.json"
WHISPER_MODEL = (
...
PARENT = load_module(
    ...
    "murmurmark_spne_v212_selector_parent",
)
AUDIO = load_module(
    ROOT / "scripts/speaker-preserving-neural-echo-v2-12-audio.py",
    "murmurmark_spne_v212_audio_runtime",
)
SHADOW = load_module(
    ROOT / "scripts/speaker-preserving-echo-full-shadow-v2-12.py",
    "murmurmark_spne_v212_shadow_runtime",
)
V28 = PARENT.V28
...
def verify_policy(path: Path) -> dict[str, Any]:
    ...
    if policy.get("schema") != "murmurmark.speaker_preserving_neural_echo_policy/v2.12":
        raise RuntimeError("unexpected v2.12 policy schema")
    pairs = (
        ...
        ("v2_11_corpus_decision", "v2_11_corpus_decision_sha256"),
        ("v2_11_corpus_report", "v2_11_corpus_report_sha256"),
    )
    checks = {
    ...
    checks["hard_set_frozen"] = (
        hard_set.get("status") == "frozen_before_v2_12_implementation"
        and hard_set.get("training_use") == "forbidden"
        and hard_set.get("threshold_tuning_use") == "forbidden"
    )
    checks["corpus_set_frozen"] = (
        corpus_set.get("status") == "frozen_before_v2_12_implementation"
    ...
    checks["v2_11_not_promoted"] = (
        V28.read_json(policy_artifact(policy, "v2_11_corpus_decision")).get("decision")
        == "DO_NOT_PROMOTE"
    )
    if not all(checks.values()):
        raise RuntimeError(f"v2.12 policy verification failed: {checks}")
    return policy
...
def outcome_rollback_chunks(
    ...
    actionable = {"chronology_not_worse", "local_recall_preserved"}
    ...
    if "chronology_not_worse" in failed:
        ...
        for row in candidate:
            if float(row.get("duration_sec") or 0.0) <= 2.0:
                continue
            if any(PARENT.overlap_match(row, other) >= 0.8 for other in baseline):
                continue
...
def archive_initial_attempt(
    ...
    payload = {
        "schema": "murmurmark.speaker_preserving_neural_echo_audio_rollback/v2.12",
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    payload["schema"] = "murmurmark.speaker_preserving_neural_echo_selection/v2.12"
    payload["selection_contract"] = (
        "window_identical_exact_local_audio_then_outcome_regression_chunk_rollback_then_full_shadow"
    )
...
def verify(args: argparse.Namespace) -> dict[str, Any]:
    ...
    checks = {
        ...
        == "murmurmark.speaker_preserving_neural_echo_selection/v2.12",
    ...
    payload = {
        "schema": "murmurmark.speaker_preserving_neural_echo_verification/v2.12",
