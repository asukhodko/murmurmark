#!/usr/bin/env python3
"""Run outcome-gated profile-matched shadow evaluation for v2.15 audio."""
...
CANDIDATE = "speaker_preserving_neural_echo_v2_15"
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    candidate_root = session / "derived/preprocess" / OUTPUT_NAME
    candidate_audio = candidate_root / "candidate_clean_mic_pcm16.wav"
    output_root = candidate_root / "full-shadow-v2-15"
    ...
    for prior_name, prior_shadow, prior_candidate in (
        (
            "speaker-preserving-neural-echo-v2-14",
            "full-shadow-v2-14",
            "speaker_preserving_neural_echo_v2_14",
        ),
        (
            "speaker-preserving-neural-echo-v2-13",
            "full-shadow-v2-13",
            "speaker_preserving_neural_echo_v2_13",
        ),
        (
            "speaker-preserving-neural-echo-v2-12",
            "full-shadow-v2-12",
            "speaker_preserving_neural_echo_v2_12",
        ),
        (
            "speaker-preserving-neural-echo-v2-11",
            "full-shadow-v2-11",
            "speaker_preserving_neural_echo_v2_11",
        ),
        (
            "speaker-preserving-neural-echo-v2-10",
            "full-shadow-v2-10",
            "speaker_preserving_neural_echo_v2_10",
        ),
        (
            "speaker-preserving-neural-echo-v2-7",
            "full-shadow-v2-9",
        ...
        prior_stage = (
            session
            / f"derived/preprocess/{prior_name}/{prior_shadow}/candidates/"
            / prior_candidate
        ...
        if prior_stage.is_dir() and not stage.exists():
            ...
            for relative_path in COMMON.MICRO_ASR_CACHE_PATHS:
                COMMON.seed_file_cache(
                    prior_stage / relative_path, stage_cache / relative_path
                )
    ...
    payload["schema"] = "murmurmark.echo_suppression_full_shadow/v2.15"
    payload["candidate_profile"] = CANDIDATE
    payload["candidate_runtime_schema"] = (
        "murmurmark.speaker_preserving_neural_echo_runtime/v2.15"
    )
    ...
    payload["gates"]["needs_review_not_worse"] = int(
        payload.get("metrics", {}).get("needs_review_count_candidate") or 0
    ) <= int(payload.get("metrics", {}).get("needs_review_count_baseline") or 0)
