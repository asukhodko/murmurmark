...
EOF
  ...
  echo "$acceptance_live" | grep -q '^  scope: lab proof and existing-session comparison only while real sidecar capture is quarantined$'
  ...
  echo "$acceptance_live" | grep -q '^    - murmurmark live pilot sessions/<existing-live-session> --controlled-real$'
  echo "$acceptance_live" | grep -q '^    - murmurmark experiment status sessions/<existing-live-session>$'
  echo "$acceptance_live" | grep -q '^    - murmurmark experiment report sessions/<existing-live-session>$'
  echo "$acceptance_live" | grep -q '^    - murmurmark experiment compare sessions/<existing-live-session> --experiment live-shadow-v1$'
  echo "$acceptance_live" | grep -q '^    - pilot runner writes derived/live/live_parity_pilot_report.json for lab sessions$'
  echo "$acceptance_live" | grep -q '^    - controlled real recording refuses to start without --allow-unsafe-controlled-real-recording$'
