...
EOF
  ...
  echo "$acceptance_live" | grep -q '^    - murmurmark live pilot --controlled-real --skip-safety-gate$'
  echo "$acceptance_live" | grep -q '^    - murmurmark experiment status latest$'
  echo "$acceptance_live" | grep -q '^    - murmurmark experiment report latest$'
  echo "$acceptance_live" | grep -q '^    - murmurmark experiment compare latest --experiment live-shadow-v1$'
