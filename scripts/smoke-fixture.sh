...
PY
  ...
  jq '.utterances += [
    ...
    {id: "utt_reused_id_me", start: 102.0, end: 104.0, source_track: "mic", speaker_label: "Me", role: "Me", text: "Отправил в чат.", quality: {needs_review: false}},
...
{"schema":"murmurmark.audio_review_audit/v1","id":"arp_stale_reused_id","session_id":"group-fixture","profile":"audit_cleanup_v1","interval":{"start":110.0,"end":112.0,"duration_sec":2.0,"start_time":"01:50","end_time":"01:52"},"source_reasons":["stale_id_fixture"],"utterance_ids":["utt_reused_id_me"],"utterances":[{"id":"utt_reused_id_me","role":"Me","source_track":"mic","start":110.0,"end":112.0,"text":"Отправил в чат.","needs_review":true}],"features":{"text":{"similarity":0.0,"containment":0.0,"me_text":"Отправил в чат.","remote_text":""}},"scores":{"local_support":0,"remote_similarity":0,"remote_duplicate":0,"remote_leak":0,"asr_noise":70},"classification":{"label":"uncertain","verdict":"needs_stronger_audio_judge","confidence":0.69,"reason":"stale utterance id fixture","top_score":70,"second_score":0}}
...
EOF
  ...
  jq -e 'any(.utterances[]; .id == "utt_reused_id_me" and (.quality.audit_cleanup | not))' "$cleanup_v2_dialogue" >/dev/null
  ...
  jq -s 'any(.[]; .target.utterance_id == "utt_reused_id_me" and .reason == "stale_audio_review_utterance_identity" and .safety_checks.utterance_identity_valid == false)' "$cleanup_v2_rejected" >/dev/null
...
EOF
  ...
  echo "$acceptance_live" | grep -q '^    - murmurmark live gate$'
  echo "$acceptance_live" | grep -q '^    - murmurmark corpus live all --refresh$'
  echo "$acceptance_live" | grep -q '^    - experiment state reports live_preview_mode committed_pcm_queue_v1$'
  echo "$acceptance_live" | grep -q '^    - derived/live/segments.jsonl points to derived/experiments/live-shadow-v1/audio files$'
  echo "$acceptance_live" | grep -q '^    - experiment compare writes live-vs-batch evidence without promoting live$'
...
PY
  ...
  echo "$explicit_local_recall_apply_output" | grep -q '^  next_lane: check_transcript_text'
  echo "$explicit_local_recall_apply_output" | grep -q '^  next:$'
  echo "$explicit_local_recall_apply_output" | grep -q '^    murmurmark review lane check_transcript_text --session '
  echo "$explicit_local_recall_apply_output" | grep -q '^    murmurmark review lane apply check_transcript_text --session '
  ...
  echo "$explicit_local_recall_apply_output" | grep -q '^next: murmurmark review lane check_transcript_text --session '
  ...
  echo "$explicit_progress_output" | grep -q '^  next_lane: check_transcript_text'
  echo "$explicit_progress_output" | grep -q '^  recommended_next: murmurmark review lane check_transcript_text --session '
  echo "$explicit_progress_output" | grep -q '^  next:$'
  echo "$explicit_progress_output" | grep -q '^    murmurmark review lane check_transcript_text --session '
  echo "$explicit_progress_output" | grep -q '^    murmurmark review lane apply check_transcript_text --session '
