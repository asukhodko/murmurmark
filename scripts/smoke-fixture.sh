...
echo "$review_apply_not_ready_output" | grep -q '^    murmurmark review first-lane --session .*review-next-session'
echo "$review_apply_not_ready_output" | grep -q '^    murmurmark review lane apply first --session .*review-next-session'
