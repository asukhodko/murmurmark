#!/usr/bin/env python3
"""Lock and evaluate the physical hypothesis-bank echo candidate v2.2."""
...
POLICY_PATH = ROOT / "policies/speaker-preserving-neural-echo-v2-2.json"
OUTPUT = ROOT / "sessions/_reports/speaker-preserving-neural-echo-v2-2"
...
V21 = load_module(V21_PATH, "murmurmark_spne_v22_v21_helpers")


def verify_policy(path: Path) -> dict[str, Any]:
    policy = read_json(path)
    if policy.get("schema") != "murmurmark.speaker_preserving_neural_echo_policy/v2.2":
        raise RuntimeError("unexpected v2.2 policy schema")
    source = policy["source"]
    pairs = (
        ("training_policy", "training_policy_sha256"),
        ("previous_policy", "previous_policy_sha256"),
        ("previous_candidate_lock", "previous_candidate_lock_sha256"),
        ("previous_hard_decision", "previous_hard_decision_sha256"),
        ("checkpoint", "checkpoint_sha256"),
        ("arbitration", "arbitration_sha256"),
        ("hypothesis_bank", "hypothesis_bank_sha256"),
    ...
    previous_decision = read_json(ROOT / str(source["previous_hard_decision"]))
    hard_manifest = read_json(ROOT / str(source["hard_manifest"]))
    semantic = {
        "previous_candidate_rejected": previous_decision.get("decision") == "DO_NOT_PROMOTE",
        ...
        == sorted(policy["hard_isolation"]["source_sessions"]),
        "hypothesis_bank_matches_policy": list(BANK.ADDITIONAL_FIR_MULTIPLIERS)
    ...
    if not passed:
        raise RuntimeError("v2.2 frozen source verification failed")
...
def apply_model_batch(model: Any, values: np.ndarray) -> np.ndarray:
    return V21.apply_model_batch(model, values)


def select_row(
    row: np.ndarray, neural_candidate: np.ndarray
...
def evaluate_waveforms(
    ...
    for start in range(0, values.shape[0], 16):
        ...
        neural = apply_model_batch(model, batch)
        for row, neural_candidate in zip(batch, neural):
            selected, decision = select_row(row, neural_candidate)
...
def controlled_dev(model: Any) -> dict[str, Any]:
    ...
    neural = sf.read(
        CONTROLLED_VALIDATION / "controlled_double_talk.candidate.wav", dtype="float32"
    )[0]
    count = min(baseline.size, raw.size, remote.size, neural.size)
    ...
    for start in range(0, count, chunk):
        ...
        selected, decision = BANK.select(
            ...
            neural_candidate=neural[start:end],
        )
...
def command_evaluate_dev(args: argparse.Namespace) -> int:
    ...
    evaluation, _ = evaluate_waveforms(synthetic, model)
    negative_exact = True
    negative_count = 0
    for kind in (
        ...
        if not subset.size:
            continue
        neural = apply_model_batch(model, subset)
        for row, neural_candidate in zip(subset, neural):
            selected, _ = select_row(row, neural_candidate)
            negative_count += 1
            negative_exact = negative_exact and np.array_equal(selected, row[0])
    controlled = controlled_dev(model)
    ...
    report = {
        "schema": "murmurmark.speaker_preserving_neural_echo_dev_evaluation/v2.2",
...
def command_lock(args: argparse.Namespace) -> int:
    ...
    if report.get("passed") is not True or report.get("policy_sha256") != CORE.sha256(args.policy):
        raise RuntimeError("v2.2 dev evaluation is stale or failed")
    ...
    if replay["evaluation"]["candidate_output_stream_sha256"] != first_hash:
        raise RuntimeError("v2.2 dev replay is not deterministic")
    if replay["controlled_dev"]["output_sha256"] != first_controlled_hash:
        raise RuntimeError("v2.2 controlled replay is not deterministic")
    policy = verification["policy"]
    basis = {
        "policy_sha256": CORE.sha256(args.policy),
        "evaluator_sha256": CORE.sha256(Path(__file__)),
        "arbitration_sha256": CORE.sha256(Path(ARBITER.__file__)),
        "hypothesis_bank_sha256": CORE.sha256(Path(BANK.__file__)),
    ...
    lock = {
        "schema": "murmurmark.speaker_preserving_neural_echo_candidate_lock/v2.2",
    ...
    write_json(
        ...
        {
            "schema": "murmurmark.speaker_preserving_neural_echo_hard_unlock/v2.2",
...
def command_hard_test(args: argparse.Namespace) -> int:
    ...
    if unlock.get("attempts_consumed") != 0:
        raise RuntimeError("v2.2 hard-test attempt already consumed")
    if CORE.sha256(lock_path) != unlock.get("candidate_lock_sha256"):
        raise RuntimeError("v2.2 lock changed")
    frozen = {
        "evaluator_sha256": CORE.sha256(Path(__file__)),
        "arbitration_sha256": CORE.sha256(Path(ARBITER.__file__)),
        "hypothesis_bank_sha256": CORE.sha256(Path(BANK.__file__)),
    ...
    if any(lock["basis"].get(key) != value for key, value in frozen.items()):
        raise RuntimeError("v2.2 implementation changed after lock")
    attempt_path = args.output / "hard_test_attempt.json"
    if attempt_path.exists():
        raise RuntimeError("v2.2 hard-test marker already exists")
    attempt = {
        "schema": "murmurmark.speaker_preserving_neural_echo_hard_attempt/v2.2",
        "status": "started",
        "candidate_lock_fingerprint": lock["fingerprint"],
    }
    ...
    if CORE.sha256(cache_path) != policy["source"]["hard_cache_sha256"]:
        raise RuntimeError("v2.2 hard cache changed")
    values = np.load(cache_path)["waveforms"].astype(np.float32)
    model, checkpoint_path = checkpoint(policy)
    evaluation, selected_audio = evaluate_waveforms(values, model)
    ...
    report = {
        "schema": "murmurmark.speaker_preserving_neural_echo_hard_evaluation/v2.2",
    ...
    write_json(attempt_path, attempt)
    decision = {
        "schema": "murmurmark.speaker_preserving_neural_echo_hard_decision/v2.2",
        "decision": "HARD_TEST_PASSED_V2_2" if report["passed"] else "DO_NOT_PROMOTE",
        "candidate_lock_fingerprint": lock["fingerprint"],
