#!/usr/bin/env python3
"""Select v2.15 audio after bounded iterative pre-ASR outcome rollback."""
...
POLICY_PATH = ROOT / "policies/speaker-preserving-neural-echo-v2-15.json"
WHISPER_MODEL = (
    Path.home() / ".local/share/murmurmark/models/whisper.cpp/ggml-large-v3-q5_0.bin"
)
OUTPUT_NAME = "speaker-preserving-neural-echo-v2-15"
...
def verify_policy(path: Path) -> dict[str, Any]:
    ...
    if policy.get("schema") != "murmurmark.speaker_preserving_neural_echo_policy/v2.15":
        raise RuntimeError("unexpected v2.15 policy schema")
    pairs = (
        ...
        ("v2_14_hard_decision", "v2_14_hard_decision_sha256"),
        ("v2_14_hard_report", "v2_14_hard_report_sha256"),
    )
    checks = {
    ...
    checks["hard_set_frozen"] = (
        hard_set.get("status") == "frozen_before_v2_15_implementation"
        and hard_set.get("training_use") == "forbidden"
        and hard_set.get("threshold_tuning_use") == "forbidden"
    )
    checks["corpus_set_frozen"] = (
        corpus_set.get("status") == "frozen_before_v2_15_implementation"
    ...
    checks["policy_status"] = policy.get("status") in {
        "development_candidate_before_hard_test",
        "locked_before_one_shot_hard_test",
    }
    checks["window_rollback_context"] = (
        float(policy.get("outcome_window_context_sec") or 0.0)
        == OUTCOME_WINDOW_CONTEXT_SEC
    )
    checks["v2_14_rejected"] = (
        V28.read_json(policy_artifact(policy, "v2_14_hard_decision")).get("decision")
        == "HARD_TEST_REJECTED_V2_14"
    )
    if not all(checks.values()):
        raise RuntimeError(f"v2.15 policy verification failed: {checks}")
    return policy
...
def archive_initial_attempt(
    ...
    payload = {
        "schema": "murmurmark.speaker_preserving_neural_echo_audio_rollback/v2.15",
    ...
    try:
        return AUDIO.run_session(
            SimpleNamespace(
                ...
                output=ROOT
                / "sessions/_reports/speaker-preserving-neural-echo-v2-15",
...
def archive_rollback_iteration(
    ...
    *,
    ...
    payload = {
        "schema": "murmurmark.speaker_preserving_neural_echo_rollback_iteration/v2.15",
        "pass": pass_index,
...
def applicability_classification(payload: dict[str, Any]) -> str:
    if payload.get("status") == "candidate":
        return "applicable_candidate"
    source = payload.get("source_runtime")
    source_reason = source.get("reason") if isinstance(source, dict) else None
    if source_reason == "no_asr_audited_improvement":
        return "not_applicable_exact_fallback"
    return "safety_exact_fallback"
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    source = payload.get("source_runtime")
    source_reason = source.get("reason") if isinstance(source, dict) else None
    applicability = applicability_classification(payload)
    payload["applicability"] = {
        "classification": applicability,
        "source_reason": source_reason,
        "promotion_credit": payload.get("status") == "candidate",
    }
    payload["schema"] = "murmurmark.speaker_preserving_neural_echo_selection/v2.15"
...
def verify(args: argparse.Namespace) -> dict[str, Any]:
    ...
    checks = {
        ...
        == "murmurmark.speaker_preserving_neural_echo_selection/v2.15",
    ...
    payload = {
        "schema": "murmurmark.speaker_preserving_neural_echo_verification/v2.15",
