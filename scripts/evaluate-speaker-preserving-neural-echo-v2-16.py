#!/usr/bin/env python3
"""Seal and evaluate the independent v2.16 hard set and promotion corpus.

v2.16 deliberately reuses the immutable v2.15 audio selector. Its only
algorithmic change is the evaluation contract: a bit-exact safety fallback is
a valid terminal safety disposition, but earns no utility credit.
"""
...
POLICY = ROOT / "policies/speaker-preserving-neural-echo-v2-16-evaluation.json"
HARD_OUTPUT = ROOT / "sessions/_reports/speaker-preserving-neural-echo-v2-16-hard"
CORPUS_OUTPUT = ROOT / "sessions/_reports/speaker-preserving-neural-echo-v2-16-corpus"
WHISPER_MODEL = (
    Path.home() / ".local/share/murmurmark/models/whisper.cpp/ggml-large-v3-q5_0.bin"
)
HARD_PASS = "HARD_TEST_PASSED_V2_16"
HARD_REJECT = "HARD_TEST_REJECTED_V2_16"
...
SELECTOR = load_module(
    ...
    "murmurmark_spne_v216_evaluation_selector",
)
...
def verify_policy(path: Path) -> dict[str, Any]:
    ...
    if policy.get("schema") != "murmurmark.speaker_preserving_neural_echo_evaluation_policy/v2.16":
        raise RuntimeError("unexpected v2.16 evaluation policy schema")
    checks = {}
    for key, hash_key in (
        ...
        ("prior_v2_15_hard_report", "prior_v2_15_hard_report_sha256"),
        ("prior_v2_15_hard_decision", "prior_v2_15_hard_decision_sha256"),
    ):
    ...
    checks["sets_frozen"] = (
        hard.get("status") == "frozen_before_v2_16_implementation"
        and corpus.get("status") == "frozen_before_v2_16_implementation"
    )
    checks["sets_disjoint"] = bool(hard_ids) and bool(corpus_ids) and hard_ids.isdisjoint(corpus_ids)
    checks["policy_locked"] = policy.get("status") == "locked_before_one_shot_hard_test"
    checks["selector_policy_locked"] = (
        selector_policy.get("status") == "locked_before_one_shot_hard_test"
    )
    prior_report = read_json(policy_artifact(policy, "prior_v2_15_hard_report"))
    prior_decision = read_json(policy_artifact(policy, "prior_v2_15_hard_decision"))
    prior_failed = [
        name for name, passed in prior_report.get("checks", {}).items() if passed is not True
    ]
    prior_failed_rows = []
    for row in prior_report.get("rows", []):
        failed_row_checks = [
            name for name, passed in row.get("checks", {}).items() if passed is not True
        ]
        if failed_row_checks:
            prior_failed_rows.append(
                {
                    "applicability": row.get("applicability"),
                    "failed_checks": failed_row_checks,
                }
            )
    checks["v2_15_rejection_preserved"] = (
        prior_decision.get("decision") == "HARD_TEST_REJECTED_V2_15"
        and prior_decision.get("report_sha256")
        == policy.get("prior_v2_15_hard_report_sha256")
        and prior_report.get("passed") is False
        and set(prior_failed)
        == {"all_sessions_passed", "speaker_sessions_classified_min"}
        and prior_failed_rows
        == [
            {
                "applicability": "safety_exact_fallback",
                "failed_checks": ["speaker_applicability_classified"],
            }
        ]
    )
    checks["contract_change_only"] = policy.get("contract_change") == (
        "accept_bit_exact_safety_fallback_as_terminal_without_utility_credit"
    )
    checks["zero_post_asr_credit"] = policy.get("post_asr_cleanup_promotion_credit") == 0
    if not all(checks.values()):
        raise RuntimeError(f"v2.16 evaluation policy verification failed: {checks}")
    return policy
...
def verify_manifest(output: Path, kind: str) -> dict[str, Any]:
    ...
    expected = f"murmurmark.speaker_preserving_neural_echo_{kind}_manifest/v2.16"
    if manifest.get("schema") != expected:
        raise RuntimeError(f"seal the v2.16 {kind} inputs first")
    if stable_digest(manifest.get("basis")) != manifest.get("fingerprint"):
        raise RuntimeError(f"v2.16 {kind} manifest fingerprint changed")
    ...
    for group in groups:
        for item in group:
            ...
            if not path.is_file() or sha256(path) != item["sha256"]:
                raise RuntimeError(f"v2.16 {kind} input changed: {path}")
    return manifest
...
def seal(args: argparse.Namespace, kind: str) -> dict[str, Any]:
    ...
    artifacts = [
        ...
        policy_artifact(policy, "prior_v2_15_hard_report"),
        policy_artifact(policy, "prior_v2_15_hard_decision"),
    ...
    for row in set_payload["sessions"]:
        ...
        if missing:
            raise RuntimeError(f"{session.name}: missing v2.16 {kind} inputs: {missing}")
        mode = detected_mode(session)
        if mode != row["expected_mode"]:
            raise RuntimeError(
                f"{session.name}: expected {row['expected_mode']}, got {mode}"
            )
        candidate = session / "derived/preprocess/speaker-preserving-neural-echo-v2-15"
        if kind == "hard" and candidate.exists():
            raise RuntimeError(
                f"{session.name}: immutable v2.15 selector output exists before {kind} seal"
            )
        sessions.append(
            {
    ...
    payload = {
        "schema": f"murmurmark.speaker_preserving_neural_echo_{kind}_manifest/v2.16",
        "status": f"sealed_before_v2_16_{kind}_evaluation",
        "basis": basis,
    ...
    if existing and existing != payload:
        raise RuntimeError(f"v2.16 {kind} manifest already has different inputs")
...
def lock_hard(args: argparse.Namespace) -> dict[str, Any]:
    ...
    payload = {
        "schema": "murmurmark.speaker_preserving_neural_echo_candidate_lock/v2.16",
    ...
    existing = read_json(destination)
    if existing and existing != payload:
        raise RuntimeError("v2.16 candidate lock already differs")
    write_json(destination, payload)
    unlock = {
        "schema": "murmurmark.speaker_preserving_neural_echo_hard_unlock/v2.16",
        "attempts_allowed": 1,
        "attempts_consumed": 0,
    ...
    if existing_unlock and existing_unlock != unlock:
        raise RuntimeError("v2.16 hard unlock already differs")
        ...
        rows.append(
            {
                ...
                "selection_report": fingerprint(
                    session
                    / "derived/preprocess/speaker-preserving-neural-echo-v2-15/selection_report.json"
                ),
            }
        )
    return rows
...
def run_hard(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if lock.get("basis", {}).get("evaluator_sha256") != sha256(Path(__file__)):
        raise RuntimeError("v2.16 evaluator changed after lock")
    if unlock.get("attempts_consumed") != 0:
        raise RuntimeError("v2.16 hard-test attempt already consumed")
    if sha256(args.hard_output / "candidate_lock.json") != unlock.get("candidate_lock_sha256"):
        raise RuntimeError("v2.16 candidate lock changed")
    attempt_path = args.hard_output / "hard_test_attempt.json"
    if attempt_path.exists():
        raise RuntimeError("v2.16 hard-test marker already exists")
    write_json(
        attempt_path,
        {
            "schema": "murmurmark.speaker_preserving_neural_echo_hard_attempt/v2.16",
            "status": "started",
    ...
    passed = all(checks.values())
    report = {
        "schema": "murmurmark.speaker_preserving_neural_echo_hard_evaluation/v2.16",
        "hard_fingerprint": manifest["fingerprint"],
        "candidate_lock_fingerprint": lock["fingerprint"],
        "rows": rows,
        "aggregate": aggregate,
    ...
    decision = {
        "schema": "murmurmark.speaker_preserving_neural_echo_hard_decision/v2.16",
...
def run_corpus(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if hard.get("decision") != HARD_PASS:
        raise RuntimeError("v2.16 corpus is blocked until the hard test passes")
    ...
    if (
        ...
        raise RuntimeError("v2.16 hard-test evidence is missing or changed")
    verify_manifest(args.hard_output, "hard")
    ...
    passed = all(checks.values())
    report = {
        "schema": "murmurmark.speaker_preserving_neural_echo_corpus_report/v2.16",
    ...
    write_json(
        ...
        {
            "schema": "murmurmark.speaker_preserving_neural_echo_promotion_decision/v2.16",
...
def verify_result(args: argparse.Namespace, kind: str) -> dict[str, Any]:
    ...
    payload = {
        "schema": f"murmurmark.speaker_preserving_neural_echo_{kind}_verification/v2.16",
