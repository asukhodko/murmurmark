#!/usr/bin/env python3
"""Freeze and evaluate the v2.7 pre-ASR echo candidate on ordinary sessions."""
...
POLICY_PATH = ROOT / "policies/speaker-preserving-neural-echo-corpus-v2-7.json"
OUTPUT = ROOT / "sessions/_reports/speaker-preserving-neural-echo-v2-7-corpus"
WHISPER_MODEL = (
...
RUNTIME = load_module(
    ...
    "murmurmark_spne_v27_corpus_runtime",
)
SHADOW = load_module(
    ROOT / "scripts/speaker-preserving-echo-full-shadow-v2-7.py",
    "murmurmark_spne_v27_corpus_shadow",
)
...
def verify_policy(path: Path) -> dict[str, Any]:
    ...
    if policy.get("schema") != "murmurmark.speaker_preserving_neural_echo_corpus_policy/v2.7":
...
def command_freeze(args: argparse.Namespace) -> int:
    ...
    payload = {
        "schema": "murmurmark.speaker_preserving_neural_echo_frozen_corpus/v2.7",
...
def verify_frozen(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if frozen.get("schema") != "murmurmark.speaker_preserving_neural_echo_frozen_corpus/v2.7":
...
def baseline_remote_supported(session: Path) -> dict[str, Any]:
    ...
    return RUNTIME.remote_supported_burden(mic, remote, states)


def fallback_metrics(session: Path) -> dict[str, Any]:
    remote = baseline_remote_supported(session)
    return {
        "local_retention": {
            "baseline_tokens": 0,
            "matched_tokens": 0,
...
def run_one(
    ...
    strict_candidate_gates = {
        ...
        "remote_supported_not_increased": float(source_after.get("seconds") or 0.0)
        <= float(source_before.get("seconds") or 0.0)
        + float(direct["remote_supported_seconds_increase_per_session_max"]),
        "remote_supported_tokens_reduced": int(
            source_metrics.get("remote_supported_token_reduction") or 0
        )
        >= int(direct["remote_supported_token_reduction_per_candidate_min"]),
    }
    ...
    gates = {
        ...
        <= int(direct["reviewed_me_token_regressions_max"]),
        "remote_supported_not_increased": float(after.get("seconds") or 0.0)
        <= float(before.get("seconds") or 0.0)
        + float(direct["remote_supported_seconds_increase_per_session_max"]),
        "remote_supported_tokens_reduced": status == "fallback"
        or int(metrics.get("remote_supported_token_reduction") or 0)
        >= int(direct["remote_supported_token_reduction_per_candidate_min"]),
    ...
    payload = {
        "schema": "murmurmark.speaker_preserving_neural_echo_corpus_session/v2.7",
        "session": session_id,
        ...
        "direct_asr": {
            ...
            "remote_supported_baseline": before,
            "remote_supported_candidate": after,
...
def command_run(args: argparse.Namespace) -> int:
    ...
    for session_id in selected_ids(args, frozen):
        ...
        summary = {
            ...
            "remote_supported_reduction_sec": payload["direct_asr"][
...
def render_report(payload: dict[str, Any]) -> str:
    ...
    lines = [
        "# Speaker-Preserving Neural Echo v2.7 Corpus",
        "",
        ...
        f"- Remote-supported mic burden: "
        f"`{aggregate['remote_supported_seconds_baseline']:.3f}s` -> "
        f"`{aggregate['remote_supported_seconds_candidate']:.3f}s`",
        f"- Remote-supported reduction: "
        f"`{aggregate['remote_supported_seconds_reduction']:.3f}s` "
        f"(`{aggregate['remote_supported_seconds_reduction_ratio']:.3%}`), "
        f"`{aggregate['remote_supported_token_reduction']}` tokens",
        "- Post-ASR cleanup promotion credit: `0`",
        "",
        "| Session | Runtime | Local | Remote-supported delta | Tokens | Full shadow |",
        "|---|---|---:|---:|---:|---|",
    ]
    for row in payload["sessions"]:
        lines.append(
            ...
            f"{row['direct_asr']['remote_supported_reduction_sec']:+.3f}s | "
            f"{row['direct_asr']['remote_supported_token_reduction']:+d} | "
...
def command_report(args: argparse.Namespace) -> int:
    ...
    for session_id in ids:
        ...
        if payload.get("schema") != "murmurmark.speaker_preserving_neural_echo_corpus_session/v2.7":
    ...
    baseline_remote = sum(
        float(row["direct_asr"]["remote_supported_baseline"].get("seconds") or 0.0)
        for row in rows
    )
    candidate_remote = sum(
        float(row["direct_asr"]["remote_supported_candidate"].get("seconds") or 0.0)
        for row in rows
    )
    reduction = baseline_remote - candidate_remote
    reduction_ratio = reduction / max(baseline_remote, 1.0e-9)
    positive = sum(
        row["direct_asr"]["remote_supported_reduction_sec"] > 0.0 for row in rows
    )
    token_reduction = sum(
        int(row["direct_asr"]["remote_supported_token_reduction"]) for row in rows
    )
    ...
    gates = {
        ...
        "aggregate_remote_supported_reduction_seconds": reduction
        >= float(promotion["aggregate_remote_supported_reduction_sec_min"]),
        "aggregate_remote_supported_reduction_ratio": reduction_ratio
        >= float(promotion["aggregate_remote_supported_reduction_ratio_min"]),
        "aggregate_remote_supported_token_reduction": token_reduction
        >= int(promotion["aggregate_remote_supported_token_reduction_min"]),
        "enough_positive_sessions": positive
        >= int(promotion["sessions_with_remote_supported_reduction_min"]),
    ...
    aggregate = {
        ...
        "remote_supported_seconds_baseline": round(baseline_remote, 3),
        "remote_supported_seconds_candidate": round(candidate_remote, 3),
        "remote_supported_seconds_reduction": round(reduction, 3),
        "remote_supported_seconds_reduction_ratio": round(reduction_ratio, 6),
        "remote_supported_token_reduction": token_reduction,
    ...
    payload = {
        "schema": "murmurmark.speaker_preserving_neural_echo_corpus_report/v2.7",
        "frozen_fingerprint": frozen["fingerprint"],
        "aggregate": aggregate,
        "sessions": rows,
        "promotion": {
...
def command_verify(args: argparse.Namespace) -> int:
    ...
    payload = {
        "schema": "murmurmark.speaker_preserving_neural_echo_corpus_verification/v2.7",
