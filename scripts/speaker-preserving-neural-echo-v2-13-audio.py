#!/usr/bin/env python3
"""Materialize v2.13 audio with window-identical candidate ASR preparation."""
...
PARENT = load_module(
    ...
    "murmurmark_spne_v213_audio_parent",
)
V210 = PARENT.PARENT
V27 = V210.V27
...
def seed_prior_cache(session: Path, output: Path) -> dict[str, int]:
    source = session / "derived/preprocess/speaker-preserving-neural-echo-v2-12"
    copied_files = 0
    for name in (
...
def transcribe_window_identical(
    *,
    ...
    payload["params"].update(
        {
            ...
            "murmurmark_echo_profile": "speaker_preserving_neural_echo_v2_13",
    ...
    V27.write_json(
        ...
        {
            "schema": "murmurmark.speaker_preserving_neural_echo_chunk_asr/v2.13",
...
def run_session(args: argparse.Namespace) -> dict[str, Any]:
    ...
    excluded_chunks = sorted(
        {
            int(value)
            for value in getattr(args, "excluded_chunks", [])
            if int(value) > 0
        }
    )
    ...
    reuse_selection = (
        bool(excluded_chunks)
        and selected_path.is_file()
        and decisions_path.is_file()
    )
    cached_selected = V27.read_jsonl(selected_path) if reuse_selection else []
    ...
    payload["schema"] = "murmurmark.speaker_preserving_neural_echo_runtime/v2.13"
    ...
    payload["v2_12_cache_seed"] = seeded
    payload["diagnostic_selection_reuse"] = {
        "used": reuse_selection,
        "reason": "excluded_chunks_only_remove_previously_audited_windows"
        if reuse_selection
...
def main() -> int:
    ...
    args.output = ROOT / "sessions/_reports/speaker-preserving-neural-echo-v2-13"
