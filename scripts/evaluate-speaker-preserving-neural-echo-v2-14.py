#!/usr/bin/env python3
"""Seal and evaluate the independent v2.14 hard set and promotion corpus."""
...
POLICY = ROOT / "policies/speaker-preserving-neural-echo-v2-14-evaluation.json"
HARD_OUTPUT = ROOT / "sessions/_reports/speaker-preserving-neural-echo-v2-14-hard"
CORPUS_OUTPUT = ROOT / "sessions/_reports/speaker-preserving-neural-echo-v2-14-corpus"
WHISPER_MODEL = (
    Path.home() / ".local/share/murmurmark/models/whisper.cpp/ggml-large-v3-q5_0.bin"
)
HARD_PASS = "HARD_TEST_PASSED_V2_14"
HARD_REJECT = "HARD_TEST_REJECTED_V2_14"
...
SELECTOR = load_module(
    ...
    "murmurmark_spne_v214_evaluation_selector",
)
...
def verify_policy(path: Path) -> dict[str, Any]:
    ...
    if policy.get("schema") != "murmurmark.speaker_preserving_neural_echo_evaluation_policy/v2.14":
        raise RuntimeError("unexpected v2.14 evaluation policy schema")
    checks = {}
    for key, hash_key in (
    ...
    checks["sets_frozen"] = (
        hard.get("status") == "frozen_before_v2_14_implementation"
        and corpus.get("status") == "frozen_before_v2_14_implementation"
    )
    ...
    if not all(checks.values()):
        raise RuntimeError(f"v2.14 evaluation policy verification failed: {checks}")
    return policy
...
def verify_manifest(output: Path, kind: str) -> dict[str, Any]:
    ...
    expected = f"murmurmark.speaker_preserving_neural_echo_{kind}_manifest/v2.14"
    if manifest.get("schema") != expected:
        raise RuntimeError(f"seal the v2.14 {kind} inputs first")
    if stable_digest(manifest.get("basis")) != manifest.get("fingerprint"):
        raise RuntimeError(f"v2.14 {kind} manifest fingerprint changed")
    ...
    for group in groups:
        for item in group:
            ...
            if not path.is_file() or sha256(path) != item["sha256"]:
                raise RuntimeError(f"v2.14 {kind} input changed: {path}")
    return manifest
...
def seal(args: argparse.Namespace, kind: str) -> dict[str, Any]:
    ...
    for row in set_payload["sessions"]:
        ...
        if missing:
            raise RuntimeError(f"{session.name}: missing v2.14 {kind} inputs: {missing}")
        mode = detected_mode(session)
        if mode != row["expected_mode"]:
            raise RuntimeError(
                f"{session.name}: expected {row['expected_mode']}, got {mode}"
            )
        candidate = session / "derived/preprocess/speaker-preserving-neural-echo-v2-14"
        if kind == "hard" and candidate.exists():
            raise RuntimeError(f"{session.name}: v2.14 output exists before {kind} seal")
        sessions.append(
            {
    ...
    payload = {
        "schema": f"murmurmark.speaker_preserving_neural_echo_{kind}_manifest/v2.14",
        "status": f"sealed_before_v2_14_{kind}_evaluation",
        "basis": basis,
    ...
    if existing and existing != payload:
        raise RuntimeError(f"v2.14 {kind} manifest already has different inputs")
...
def lock_hard(args: argparse.Namespace) -> dict[str, Any]:
    ...
    payload = {
        "schema": "murmurmark.speaker_preserving_neural_echo_candidate_lock/v2.14",
    ...
    existing = read_json(destination)
    if existing and existing != payload:
        raise RuntimeError("v2.14 candidate lock already differs")
    write_json(destination, payload)
    unlock = {
        "schema": "murmurmark.speaker_preserving_neural_echo_hard_unlock/v2.14",
        "attempts_allowed": 1,
        "attempts_consumed": 0,
    ...
    if existing_unlock and existing_unlock != unlock:
        raise RuntimeError("v2.14 hard unlock already differs")
        ...
        rows.append(
            {
                ...
                "selection_report": fingerprint(
                    session
                    / "derived/preprocess/speaker-preserving-neural-echo-v2-14/selection_report.json"
                ),
            }
        )
    return rows
...
def run_hard(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if lock.get("basis", {}).get("evaluator_sha256") != sha256(Path(__file__)):
        raise RuntimeError("v2.14 evaluator changed after lock")
    if unlock.get("attempts_consumed") != 0:
        raise RuntimeError("v2.14 hard-test attempt already consumed")
    if sha256(args.hard_output / "candidate_lock.json") != unlock.get("candidate_lock_sha256"):
        raise RuntimeError("v2.14 candidate lock changed")
    attempt_path = args.hard_output / "hard_test_attempt.json"
    if attempt_path.exists():
        raise RuntimeError("v2.14 hard-test marker already exists")
    write_json(
        attempt_path,
        {
            "schema": "murmurmark.speaker_preserving_neural_echo_hard_attempt/v2.14",
            "status": "started",
    ...
    passed = all(checks.values())
    report = {
        "schema": "murmurmark.speaker_preserving_neural_echo_hard_evaluation/v2.14",
        "hard_fingerprint": manifest["fingerprint"],
        "candidate_lock_fingerprint": lock["fingerprint"],
        "rows": rows,
        "aggregate": aggregate,
    ...
    decision = {
        "schema": "murmurmark.speaker_preserving_neural_echo_hard_decision/v2.14",
...
def run_corpus(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if hard.get("decision") != HARD_PASS:
        raise RuntimeError("v2.14 corpus is blocked until the hard test passes")
    ...
    if (
        ...
        raise RuntimeError("v2.14 hard-test evidence is missing or changed")
    verify_manifest(args.hard_output, "hard")
    ...
    passed = all(checks.values())
    report = {
        "schema": "murmurmark.speaker_preserving_neural_echo_corpus_report/v2.14",
    ...
    write_json(
        ...
        {
            "schema": "murmurmark.speaker_preserving_neural_echo_promotion_decision/v2.14",
...
def verify_result(args: argparse.Namespace, kind: str) -> dict[str, Any]:
    ...
    payload = {
        "schema": f"murmurmark.speaker_preserving_neural_echo_{kind}_verification/v2.14",
