#!/usr/bin/env python3
"""Select outcome-preserving v2.11 audio or the bit-exact local-FIR baseline."""
...
POLICY_PATH = ROOT / "policies/speaker-preserving-neural-echo-v2-11.json"
WHISPER_MODEL = (
...
PARENT = load_module(
    ...
    "murmurmark_spne_v211_selector_parent",
)
AUDIO = load_module(
    ROOT / "scripts/speaker-preserving-neural-echo-v2-11-audio.py",
    "murmurmark_spne_v211_audio_runtime",
)
SHADOW = load_module(
    ROOT / "scripts/speaker-preserving-echo-full-shadow-v2-11.py",
    "murmurmark_spne_v211_shadow_runtime",
)
V28 = PARENT.V28
...
def verify_policy(path: Path) -> dict[str, Any]:
    ...
    if policy.get("schema") != "murmurmark.speaker_preserving_neural_echo_policy/v2.11":
        raise RuntimeError("unexpected v2.11 policy schema")
    pairs = (
        ...
        ("v2_10_hard_decision", "v2_10_hard_decision_sha256"),
        ("v2_10_hard_report", "v2_10_hard_report_sha256"),
    )
    checks = {
    ...
    AUDIO.PARENT.verify_policy(policy_artifact(policy, "v2_7_policy"))
    ...
    checks["v2_10_rejected"] = (
        V28.read_json(policy_artifact(policy, "v2_10_hard_decision")).get("decision")
        == "HARD_TEST_REJECTED_V2_10"
    )
    if not all(checks.values()):
        raise RuntimeError(f"v2.11 policy verification failed: {checks}")
    return policy
...
def archive_initial_attempt(
    ...
    payload = {
        "schema": "murmurmark.speaker_preserving_neural_echo_audio_rollback/v2.11",
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    payload["schema"] = "murmurmark.speaker_preserving_neural_echo_selection/v2.11"
    payload["selection_contract"] = (
        "exact_local_audio_then_outcome_regression_chunk_rollback_then_full_shadow"
    )
...
def verify(args: argparse.Namespace) -> dict[str, Any]:
    ...
    checks = {
        ...
        == "murmurmark.speaker_preserving_neural_echo_selection/v2.11",
    ...
    payload = {
        "schema": "murmurmark.speaker_preserving_neural_echo_verification/v2.11",
