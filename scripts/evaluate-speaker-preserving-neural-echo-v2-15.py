#!/usr/bin/env python3
"""Seal and evaluate the independent v2.15 hard set and promotion corpus."""
...
POLICY = ROOT / "policies/speaker-preserving-neural-echo-v2-15-evaluation.json"
HARD_OUTPUT = ROOT / "sessions/_reports/speaker-preserving-neural-echo-v2-15-hard"
CORPUS_OUTPUT = ROOT / "sessions/_reports/speaker-preserving-neural-echo-v2-15-corpus"
WHISPER_MODEL = (
    Path.home() / ".local/share/murmurmark/models/whisper.cpp/ggml-large-v3-q5_0.bin"
)
HARD_PASS = "HARD_TEST_PASSED_V2_15"
HARD_REJECT = "HARD_TEST_REJECTED_V2_15"
...
SELECTOR = load_module(
    ...
    "murmurmark_spne_v215_evaluation_selector",
)
...
def materialize_missing_asr_exports(session: Path) -> list[dict[str, Any]]:
    ...
    for source, destination in pairs:
        ...
        rows.append(
            {
                "path": relative(destination),
                "status": "materialized_before_seal",
                "source": relative(source),
            }
        )
    return rows
...
def verify_policy(path: Path) -> dict[str, Any]:
    ...
    if policy.get("schema") != "murmurmark.speaker_preserving_neural_echo_evaluation_policy/v2.15":
        raise RuntimeError("unexpected v2.15 evaluation policy schema")
    checks = {}
    for key, hash_key in (
    ...
    checks["sets_frozen"] = (
        hard.get("status") == "frozen_before_v2_15_implementation"
        and corpus.get("status") == "frozen_before_v2_15_implementation"
    )
    ...
    if not all(checks.values()):
        raise RuntimeError(f"v2.15 evaluation policy verification failed: {checks}")
    return policy
...
def verify_manifest(output: Path, kind: str) -> dict[str, Any]:
    ...
    expected = f"murmurmark.speaker_preserving_neural_echo_{kind}_manifest/v2.15"
    if manifest.get("schema") != expected:
        raise RuntimeError(f"seal the v2.15 {kind} inputs first")
    if stable_digest(manifest.get("basis")) != manifest.get("fingerprint"):
        raise RuntimeError(f"v2.15 {kind} manifest fingerprint changed")
    ...
    for group in groups:
        for item in group:
            ...
            if not path.is_file() or sha256(path) != item["sha256"]:
                raise RuntimeError(f"v2.15 {kind} input changed: {path}")
    return manifest
...
def seal(args: argparse.Namespace, kind: str) -> dict[str, Any]:
    ...
    for row in set_payload["sessions"]:
        ...
        if missing:
            raise RuntimeError(f"{session.name}: missing v2.15 {kind} inputs: {missing}")
        ...
        if kind == "hard" and candidate.exists():
            raise RuntimeError(f"{session.name}: v2.15 output exists before {kind} seal")
        sessions.append(
            {
    ...
    payload = {
        "schema": f"murmurmark.speaker_preserving_neural_echo_{kind}_manifest/v2.15",
        "status": f"sealed_before_v2_15_{kind}_evaluation",
        "basis": basis,
    ...
    if existing and existing != payload:
        raise RuntimeError(f"v2.15 {kind} manifest already has different inputs")
...
def lock_hard(args: argparse.Namespace) -> dict[str, Any]:
    ...
    payload = {
        "schema": "murmurmark.speaker_preserving_neural_echo_candidate_lock/v2.15",
    ...
    existing = read_json(destination)
    if existing and existing != payload:
        raise RuntimeError("v2.15 candidate lock already differs")
    write_json(destination, payload)
    unlock = {
        "schema": "murmurmark.speaker_preserving_neural_echo_hard_unlock/v2.15",
        "attempts_allowed": 1,
        "attempts_consumed": 0,
    ...
    if existing_unlock and existing_unlock != unlock:
        raise RuntimeError("v2.15 hard unlock already differs")
...
def evaluate_sessions(
    ...
    for frozen in manifest["basis"]["sessions"]:
        ...
        checks = {
            ...
            "speaker_applicability_classified": expected_mode != "speaker_playback"
            or applicability
            in {"applicable_candidate", "not_applicable_exact_fallback"},
...
def aggregate_checks(
    ...
    if "speaker_sessions_classified_min" in gates:
        checks["speaker_sessions_classified_min"] = len(classified_speaker) >= int(
            gates["speaker_sessions_classified_min"]
        )
    ...
    aggregate = {
        ...
        "classified_speaker_sessions": len(classified_speaker),
...
def run_hard(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if lock.get("basis", {}).get("evaluator_sha256") != sha256(Path(__file__)):
        raise RuntimeError("v2.15 evaluator changed after lock")
    if unlock.get("attempts_consumed") != 0:
        raise RuntimeError("v2.15 hard-test attempt already consumed")
    if sha256(args.hard_output / "candidate_lock.json") != unlock.get("candidate_lock_sha256"):
        raise RuntimeError("v2.15 candidate lock changed")
    attempt_path = args.hard_output / "hard_test_attempt.json"
    if attempt_path.exists():
        raise RuntimeError("v2.15 hard-test marker already exists")
    write_json(
        attempt_path,
        {
            "schema": "murmurmark.speaker_preserving_neural_echo_hard_attempt/v2.15",
            "status": "started",
    ...
    passed = all(checks.values())
    report = {
        "schema": "murmurmark.speaker_preserving_neural_echo_hard_evaluation/v2.15",
        "hard_fingerprint": manifest["fingerprint"],
        "candidate_lock_fingerprint": lock["fingerprint"],
        "rows": rows,
        "aggregate": aggregate,
    ...
    decision = {
        "schema": "murmurmark.speaker_preserving_neural_echo_hard_decision/v2.15",
...
def run_corpus(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if hard.get("decision") != HARD_PASS:
        raise RuntimeError("v2.15 corpus is blocked until the hard test passes")
    ...
    if (
        ...
        raise RuntimeError("v2.15 hard-test evidence is missing or changed")
    verify_manifest(args.hard_output, "hard")
    ...
    passed = all(checks.values())
    report = {
        "schema": "murmurmark.speaker_preserving_neural_echo_corpus_report/v2.15",
    ...
    write_json(
        ...
        {
            "schema": "murmurmark.speaker_preserving_neural_echo_promotion_decision/v2.15",
...
def verify_result(args: argparse.Namespace, kind: str) -> dict[str, Any]:
    ...
    payload = {
        "schema": f"murmurmark.speaker_preserving_neural_echo_{kind}_verification/v2.15",
