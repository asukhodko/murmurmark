...
echo "$status_output" | grep -q '^  speaker_resolution_state: unavailable$'
echo "$status_output" | grep -q '^  speaker_claim_scope: remote_role_unattributed$'
...
cat >"$live_parity_session/session.json" <<'JSON'
{
  ...
  "ended_at": "2026-07-06T00:00:20.000Z",
  "status": "completed",
  "health": {
    "summary": "ok",
    "partial": false,
...
{"schema":"murmurmark.live_chunk/v1","index":1,"start_sec":0.0,"end_sec":5.0,"created_at":"2026-07-06T00:00:07.000Z","mic":{"text":"привет проверю задачу","hard_start_sec":0.0,"hard_end_sec":5.0},"remote":{"text":"привет обсудим план","hard_start_sec":0.0,"hard_end_sec":5.0}}
{"schema":"murmurmark.live_chunk/v1","index":2,"start_sec":5.0,"end_sec":10.0,"created_at":"2026-07-06T00:00:12.000Z","mic":{"text":"","hard_start_sec":5.0,"hard_end_sec":10.0,"live_boundary_gate":{"status":"passed","reason":"too_short_for_boundary_gate"}},"remote":{"text":"","raw_text_before_boundary_gate":"привет обсудим план новый хвост","hard_start_sec":5.0,"hard_end_sec":10.0,"live_boundary_gate":{"status":"suppressed","reason":"adjacent_chunk_duplicate","duplicate_score":0.6,"current_token_recall_in_previous":0.6,"previous_token_recall_in_current":1.0}}}
JSONL
