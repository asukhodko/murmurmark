#!/usr/bin/env python3
"""Build and evaluate the segmentation-stable Target-Me echo suppressor v2.6."""
...
POLICY_PATH = ROOT / "policies/speaker-preserving-neural-echo-v2-6.json"
OUTPUT = ROOT / "sessions/_reports/speaker-preserving-neural-echo-v2-6"
ENROLLMENT_ROOT = (
...
TARGET_ME = load_module(ROOT / "scripts/audit-target-me.py", "murmurmark_spne_v26_target_me")
TRANSCRIBER = load_module(
    ROOT / "scripts/transcribe-simple-whispercpp.py", "murmurmark_spne_v26_transcriber"
)
METRICS = load_module(
    ROOT / "scripts/report-speaker-preserving-neural-echo-v2-3-corpus.py",
    "murmurmark_spne_v26_legacy_metrics",
)
...
def verify_policy(path: Path) -> dict[str, Any]:
    ...
    if policy.get("schema") != "murmurmark.speaker_preserving_neural_echo_policy/v2.6":
        raise RuntimeError("unexpected v2.6 policy schema")
    ...
    if not all(bool(row.get("passed")) for row in checks.values()):
        raise RuntimeError(f"v2.6 policy verification failed: {checks}")
...
def proposal_basis(
    *,
    ...
    return {
        "schema": "murmurmark.speaker_preserving_neural_echo_proposal_basis/v2.6",
...
def cached_proposals(
    *,
    ...
    write_json(
        manifest_path,
        {
            "schema": "murmurmark.speaker_preserving_neural_echo_proposal_manifest/v2.6",
            "basis": basis,
...
def transcribe_clip_cached(
    *,
    ...
    if force or read_json(metadata).get("basis") != basis or not output_base.with_suffix(".json").is_file():
        ...
        write_json(metadata, {"schema": "murmurmark.spne_v26_chunk_asr_cache/v1", "basis": basis})
...
def transcribe_final(
    *,
    ...
    payload["params"]["murmurmark_echo_profile"] = "speaker_preserving_neural_echo_v2_6"
    ...
    write_json(
        ...
        {
        "schema": "murmurmark.speaker_preserving_neural_echo_chunk_asr/v2.6",
...
def fallback(
    *, session: Path, output: Path, reason: str, basis: dict[str, Any], details: Any = None
    ...
    payload = {
        "schema": "murmurmark.speaker_preserving_neural_echo_runtime/v2.6",
        "status": "fallback",
        "reason": reason,
        "details": details,
        "basis": basis,
...
def run_session(args: argparse.Namespace) -> dict[str, Any]:
    ...
    output = session / "derived/preprocess/speaker-preserving-neural-echo-v2-6"
...
def verify_session(args: argparse.Namespace) -> dict[str, Any]:
    ...
    checks = {
        ...
        == "murmurmark.speaker_preserving_neural_echo_runtime/v2.6",
    ...
    payload = {
        "schema": "murmurmark.speaker_preserving_neural_echo_verification/v2.6",
