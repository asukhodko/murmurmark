...
echo "  SESSION=\"sessions/\$(date +%Y-%m-%d_%H-%M-%S)\""
echo "  murmurmark record --out \"\$SESSION\" --target-bundle system"
echo "  murmurmark inspect \"\$SESSION\""
echo "  murmurmark process \"\$SESSION\""
echo "  murmurmark status \"\$SESSION\""
echo "  murmurmark acceptance --live-session \"\$SESSION\" --report /tmp/murmurmark-live-session.json"
