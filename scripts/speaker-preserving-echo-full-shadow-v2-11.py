#!/usr/bin/env python3
"""Run outcome-gated profile-matched shadow evaluation for v2.11 audio."""
...
CANDIDATE = "speaker_preserving_neural_echo_v2_11"
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    output_root = candidate_root / "full-shadow-v2-11"
    ...
    for prior_name, prior_candidate in (
        ("speaker-preserving-neural-echo-v2-10", "speaker_preserving_neural_echo_v2_10"),
        ("speaker-preserving-neural-echo-v2-7", "speaker_preserving_neural_echo_v2_9"),
    ):
        prior_stage = (
            session
            / f"derived/preprocess/{prior_name}/full-shadow-v2-10/candidates/"
            / prior_candidate
            / "full-shadow-precomputed-session/derived/transcript-simple/whisper-cpp"
        )
        if prior_name.endswith("v2-7"):
            prior_stage = (
                session
    ...
    payload["schema"] = "murmurmark.echo_suppression_full_shadow/v2.11"
    payload["candidate_profile"] = CANDIDATE
    payload["candidate_runtime_schema"] = (
        "murmurmark.speaker_preserving_neural_echo_runtime/v2.11"
    )
