...
def filter_micro_asr_turns_covered_by_base(
    candidates: list[dict[str, Any]],
    base_turns: list[dict[str, Any]],
) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for candidate in candidates:
        ...
        duration = max(0.0, end - start)
        candidate_tokens = tokens(str(candidate.get("text") or ""))
        covering: dict[str, Any] | None = None
        for base in base_turns:
            if base.get("role") != candidate.get("role"):
                continue
            if safe_int(base.get("chunk_index")) != safe_int(candidate.get("chunk_index")):
                continue
            base_start = safe_float(base.get("start"))
            base_end = safe_float(base.get("end"), base_start)
            if duration <= 0 or interval_overlap(start, end, base_start, base_end) / duration < 0.60:
                continue
            base_tokens = tokens(str(base.get("text") or ""))
            similarity = bag_recall(candidate_tokens, base_tokens) or 0.0
            if similarity >= 0.80:
                covering = {
                    "id": base.get("id"),
        ...
        if covering is None:
            ...
            rejected.append(
                {
                    **candidate,
                    "reason": "micro_asr_already_covered_by_base_turn",
                    "covered_by_base_turn": covering,
                }
            )
...
def shadow_turn_payload(turn: dict[str, Any], added_by_policy: str | None) -> dict[str, Any]:
    ...
    for key in (
        ...
        "causal_remote_active_me_separation_v1_shadow",
        "residual_method",
        "residual_audio_guard",
...
def live_boundary_micro_asr_lab_shadow_turns(
    ...
    *,
    ...
    if candidate_source == "causal-local-only-seed-live-segment":
        ...
        rejected_prefix = "live_target_me_remote_gap_micro_asr_rejected"
        source_name = "mic_target_me_remote_gap_micro_asr_shadow"
        classifier_name = "target_me_remote_gap_micro_asr_shadow_v1"
        flag_name = "target_me_remote_gap_micro_asr_shadow"
    elif candidate_source == "live-only":
    ...
    for index, item in enumerate(items, start=1):
        ...
        if safe_float(decision.get("remote_text_recall_in_micro")) > 0.10:
            ...
            continue
        if candidate_source not in live_only_candidate_sources and safe_float(
            ...
            continue
        if candidate_source in live_only_candidate_sources and safe_float(
            decision.get("source_text_token_recall")
        ) < 0.25:
        ...
        turns.append(
            {
                ...
                flag_name: True,
                "candidate_source": candidate_source,
                "used_batch_fields_for_selection": candidate_source not in live_only_candidate_sources,
                "live_group_classifier": classifier_name,
...
def causal_remote_active_me_separation_v1_shadow_turns(
    ...
    for index, row in enumerate(rows, start=1):
        ...
        residual_guard = (
            row.get("residual_audio_guard")
            if isinstance(row.get("residual_audio_guard"), dict)
            else {}
        )
        training = (
            row.get("past_training_evidence")
            if isinstance(row.get("past_training_evidence"), dict)
            else {}
        )
        remote_active = (
            row.get("remote_active_guard")
            if isinstance(row.get("remote_active_guard"), dict)
            else {}
        )
        remote_text = (
            row.get("remote_text_guard")
            if isinstance(row.get("remote_text_guard"), dict)
            else {}
        )
        target_me = (
            row.get("target_me_evidence")
            if isinstance(row.get("target_me_evidence"), dict)
            else {}
        )
        checks = {
            "accepted": row.get("status") == "accepted",
            ...
            "recording_time_evidence": recording_time.get("status") == "passed",
            "residual_audio_guard": residual_guard.get("status") == "passed",
        ...
        context = {
            "id": row.get("id") or f"causal_remote_active_me_separation_v1_rejected_{index:06d}",
            "chunk_index": row.get("chunk_index"),
            "start": row.get("start"),
            "end": row.get("end"),
            "text": row.get("text"),
            "candidate_source": "causal-remote-active-me-separation-v1",
            "contract_checks": checks,
        }
        if not all(checks.values()):
            rejected.append(
                {
                    **context,
                    "reason": "causal_remote_active_me_separation_v1_contract_failed",
                    "failed_checks": [name for name, passed in checks.items() if not passed],
                }
            )
            continue
        ...
        turns.append(
            {
                "id": str(row.get("id") or f"causal_remote_active_me_separation_v1_{index:06d}"),
                "chunk_index": row.get("chunk_index"),
                ...
                "used_batch_fields_for_selection": False,
                "timeline_causal": True,
                "live_group_classifier": "recording_time_causal_remote_active_separation_v1",
                "micro_asr_score": row.get("score"),
                "micro_asr_remote_similarity": row.get("remote_similarity"),
                "micro_asr_remote_text_recall": row.get("remote_text_recall_in_micro"),
                "micro_asr_source_text_token_recall": row.get("source_alignment"),
                "micro_asr_json": row.get("asr_json"),
                "micro_asr_wav": row.get("wav"),
                "residual_method": row.get("residual_method"),
                "residual_audio_guard": residual_guard,
                "past_training_evidence": training,
                "remote_active_guard": remote_active,
                "remote_text_guard": remote_text,
                "target_me_evidence": target_me,
                "recording_time_evidence": recording_time,
                "selection_features": {
                    "speaker_evidence": row.get("speaker_evidence") or [],
                    "source_text": row.get("source_text"),
                    "selection_mode": row.get("selection_mode"),
                    "remote_forbidden_matches": row.get("remote_forbidden_matches") or [],
                },
            }
        )
    return (
...
def boundary_order_retime_metrics(turns: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    target_me_remote_gap_micro_asr = [
        turn for turn in turns if turn.get("target_me_remote_gap_micro_asr_shadow")
    ]
    return {
        ...
        "target_me_remote_gap_micro_asr_turn_count": len(target_me_remote_gap_micro_asr),
        "target_me_remote_gap_micro_asr_seconds": round(
            sum(
                max(0.0, safe_float(turn.get("end")) - safe_float(turn.get("start")))
                for turn in target_me_remote_gap_micro_asr
            ),
            3,
        ),
    }
...
def target_me_shadow_profile_components(
    *,
    ...
    if policy in TARGET_ME_REMOTE_GAP_MICRO_ASR_PROFILE_POLICIES:
        ...
        micro_asr_turns, rejected_covered_micro_asr_turns = filter_micro_asr_turns_covered_by_base(
            micro_asr_turns,
            live_turns + target_turns,
        )
        supplemental_turns = dedupe_supplemental_turns_by_interval(
            supplemental_turns + micro_asr_turns,
        )
    ...
    if policy in CAUSAL_REMOTE_ACTIVE_ME_SEPARATION_V1_PROFILE_POLICIES:
        ...
        supplemental_turns = dedupe_supplemental_turns_by_interval(
            supplemental_turns + remote_active_turns,
        )
...
def write_target_me_shadow_drafts(
    *,
    ...
    for policy in policies:
        ...
        causal_remote_active_v1_turns = [
            turn
            for turn in supplemental_turns
            if turn.get("causal_remote_active_me_separation_v1_shadow")
        ]
        micro_asr_turns = (
            micro_asr_lab_turns
            + micro_asr_live_only_turns
        ...
        causal_remote_active_v1_rejected_turns = [
            turn
            for turn in rejected_supplemental_turns
            if "causal_remote_active_me_separation" in str(turn.get("id") or "")
            or str(turn.get("candidate_source") or "")
        ...
        micro_asr_lab_rejected_turns = [
            turn
            for turn in micro_asr_rejected_turns
            if turn not in micro_asr_live_only_rejected_turns
            and turn not in target_me_remote_gap_micro_asr_rejected_turns
        ]
        ...
        causal_remote_active_v1_seconds = round(
            sum(
                safe_float(turn.get("end")) - safe_float(turn.get("start"))
                for turn in causal_remote_active_v1_turns
            ),
            3,
        )
        ...
        payload = {
            ...
            "metrics": {
                ...
                "target_me_remote_gap_micro_asr_added_turn_count": len(
                    target_me_remote_gap_micro_asr_turns
                ),
                ...
                "causal_remote_active_me_separation_v1_added_turn_count": len(
                    causal_remote_active_v1_turns
                ),
                "causal_remote_active_me_separation_v1_added_turn_seconds": (
                    causal_remote_active_v1_seconds
                ),
                "causal_remote_active_me_separation_v1_rejected_turn_count": len(
                    causal_remote_active_v1_rejected_turns
                ),
        ...
        lines = [
            ...
            f"- Target-Me remote-gap micro-ASR added turns: `{len(target_me_remote_gap_micro_asr_turns)}` / "
            ...
            f"- causal remote-active Me separation v1 added turns: "
            f"`{len(causal_remote_active_v1_turns)}` / `{causal_remote_active_v1_seconds}s`",
            f"- causal remote-active Me separation v1 rejected turns: "
            f"`{len(causal_remote_active_v1_rejected_turns)}`",
...
def build_target_me_shadow_profiles(
    *,
    ...
    for policy in policies:
        ...
        top_level_metrics[f"{base}_target_me_remote_gap_micro_asr_turn_count"] = retime_metrics[
            "target_me_remote_gap_micro_asr_turn_count"
        ]
        top_level_metrics[f"{base}_target_me_remote_gap_micro_asr_seconds"] = retime_metrics[
        ...
        top_level_metrics[f"{base}_target_me_remote_gap_micro_asr_added_turn_count"] = len(
            target_me_remote_gap_micro_asr_turns
        )
        top_level_metrics[f"{base}_target_me_remote_gap_micro_asr_added_turn_seconds"] = (
            target_me_remote_gap_micro_asr_seconds
        )
        ...
        top_level_metrics[f"{base}_causal_remote_active_me_separation_v1_added_turn_count"] = len(
            causal_remote_active_v1_turns
        )
        top_level_metrics[f"{base}_causal_remote_active_me_separation_v1_added_turn_seconds"] = (
            causal_remote_active_v1_seconds
        )
        top_level_metrics[f"{base}_causal_remote_active_me_separation_v1_rejected_turn_count"] = len(
            causal_remote_active_v1_rejected_turns
        )
        ...
        profiles[policy] = {
            ...
            "metrics": {
                ...
                **retime_metrics,
                ...
                "live_boundary_micro_asr_live_only_added_turn_seconds": micro_asr_live_only_seconds,
                "live_boundary_micro_asr_live_only_rejected_turn_count": len(micro_asr_live_only_rejected_turns),
                "target_me_remote_gap_micro_asr_added_turn_count": len(target_me_remote_gap_micro_asr_turns),
                "target_me_remote_gap_micro_asr_added_turn_seconds": target_me_remote_gap_micro_asr_seconds,
                "target_me_remote_gap_micro_asr_rejected_turn_count": len(
                    target_me_remote_gap_micro_asr_rejected_turns
                ),
