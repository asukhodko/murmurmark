...
struct MurmurMark {
    ...
    static func printHelp() {
        ...
        Quality and corpus maintenance:
          ...
          murmurmark corpus chronology-localization-v1 preflight|freeze|decode|evaluate|status|replay|all
                                      [--policy policies/word-level-chronology-localization-v1.json]
...
enum AuditCommands {
    static func audit(_ args: [String]) throws {
        ...
        case "remote-residual", "remote_residual":
            ...
            let auditArgs = [
                try script("audit-remote-speaker-residual-evidence-v4.py").path,
                session.path,
            ] + remaining
            try Tooling.runPath(python, auditArgs)
...
enum AuditPrinter {
    ...
    static func printRemoteResidual(session: URL, args: [String]) throws {
        ...
        let reportURL = outDir.appendingPathComponent("report.json")
        guard FileManager.default.fileExists(atPath: reportURL.path) else {
            printMissing(kind: "remote_residual", expected: reportURL)
            return
        }
        let payload = try JSONFiles.object(reportURL)
        let summary = dict(payload["summary"])
...
enum CorpusCommands {
    static func corpus(_ args: [String]) throws {
        ...
        case "chronology-localization-v1", "chronology_localization_v1":
            if forwarded.isEmpty || ArgumentEditing.hasHelpFlag(forwarded) {
                try Tooling.runPath(
                    try PythonRuntime.resolve(),
                    [try script("report-word-level-chronology-localization-v1.py").path, "--help"]
                )
                return
            }
            guard let action = forwarded.first, [
                "preflight", "freeze", "decode", "evaluate", "status", "replay", "all",
            ].contains(action) else {
                throw CLIError(
                    "chronology-localization-v1 requires preflight, freeze, decode, evaluate, "
                        + "status, replay, or all"
                )
            }
            _ = try Tooling.runPathAllowingExitCodes(
                try PythonRuntime.resolve(),
                [try script("report-word-level-chronology-localization-v1.py").path] + forwarded,
                allowedExitCodes: [0, 2]
            )
        ...
        case "remote-residual", "remote_residual":
            if ArgumentEditing.hasHelpFlag(forwarded) {
                try Tooling.runPath(
                    try PythonRuntime.resolve(),
                    [try script("report-remote-speaker-residual-evidence-v4-corpus.py").path, "--help"]
                )
                return
            }
            var residualArgs = forwarded
            if !ArgumentEditing.hasOption("frozen-manifest", in: residualArgs),
               !ArgumentEditing.hasOption("write-manifest", in: residualArgs) {
                let frozenManifest = PathURLs.fileURL(
                    "docs/testing/remote-speaker-residual-evidence-v4-manifest.json"
                )
                if FileManager.default.fileExists(atPath: frozenManifest.path) {
                    residualArgs += ["--frozen-manifest", frozenManifest.path]
                }
            }
            _ = try Tooling.runPathAllowingExitCodes(
                try PythonRuntime.resolve(),
                [try script("report-remote-speaker-residual-evidence-v4-corpus.py").path]
                    + residualArgs
                    + ["--sessions-root", sessionsRoot.path],
                allowedExitCodes: [0, 2]
            )
        ...
        case "remote-duration-v2", "remote_duration_v2":
            if ArgumentEditing.hasHelpFlag(forwarded) {
                try Tooling.runPath(
                    try PythonRuntime.resolve(),
                    [try script("evaluate-duration-aware-remote-speaker-attribution-v2.py").path, "--help"]
                )
                return
            }
            guard let action = forwarded.first else {
                throw CLIError("remote-duration-v2 requires freeze, hard-status, hard-replay, develop, evaluate-hard, status, or replay")
            }
            ...
            case "freeze":
                _ = try Tooling.runPathAllowingExitCodes(
                    python,
                    [try script("freeze-remote-speaker-hard-v2.py").path, "freeze"] + tail,
                    allowedExitCodes: [0, 2]
                )
                _ = try Tooling.runPathAllowingExitCodes(
                    python,
                    [try script("freeze-remote-speaker-hard-v2.py").path, "public-manifest"] + tail,
                    allowedExitCodes: [0, 2]
                )
            case "hard-status":
                _ = try Tooling.runPathAllowingExitCodes(
                    python,
                    [try script("freeze-remote-speaker-hard-v2.py").path, "status"] + tail,
                    allowedExitCodes: [0, 1, 2]
                )
            case "hard-replay":
                _ = try Tooling.runPathAllowingExitCodes(
                    python,
                    [try script("freeze-remote-speaker-hard-v2.py").path, "replay"] + tail,
                    allowedExitCodes: [0, 2]
                )
            default:
                _ = try Tooling.runPathAllowingExitCodes(
                    python,
                    [try script("evaluate-duration-aware-remote-speaker-attribution-v2.py").path, action] + tail,
                    allowedExitCodes: [0, 1, 2]
                )
            }
        ...
        case "remote-identity-enrollment-v1", "remote_identity_enrollment_v1":
            if ArgumentEditing.hasHelpFlag(forwarded) {
                try Tooling.runPath(
                    try PythonRuntime.resolve(),
                    [try script("evaluate-session-local-remote-speaker-enrollment-hardening-v1.py").path, "--help"]
                )
                return
            }
            guard let action = forwarded.first else {
                throw CLIError(
                    "remote-identity-enrollment-v1 requires preflight, freeze, build, evaluate, status, replay, finalize, or all"
                )
            }
            guard [
                "preflight", "freeze", "build", "evaluate", "status", "replay", "finalize", "all",
            ].contains(action) else {
                throw CLIError("unsupported remote-identity-enrollment-v1 action: \(action)")
            }
            _ = try Tooling.runPathAllowingExitCodes(
                try PythonRuntime.resolve(),
                [try script("evaluate-session-local-remote-speaker-enrollment-hardening-v1.py").path] + forwarded,
                allowedExitCodes: [0, 2]
            )
        case "remote-truth-seed-v1", "remote_truth_seed_v1":
            if ArgumentEditing.hasHelpFlag(forwarded) {
                try Tooling.runPath(
                    try PythonRuntime.resolve(),
                    [try script("build-remote-speaker-direct-truth-seed-v1.py").path, "--help"]
                )
                return
            }
            guard let action = forwarded.first else {
                throw CLIError(
                    "remote-truth-seed-v1 requires preflight, build, next, grade, status, finalize, replay, or all"
                )
            }
            guard [
                "preflight", "build", "next", "grade", "status", "finalize", "replay", "all",
            ].contains(action) else {
                throw CLIError("unsupported remote-truth-seed-v1 action: \(action)")
            }
            _ = try Tooling.runPathAllowingExitCodes(
                try PythonRuntime.resolve(),
        ...
        case "remote-reclustering-v1", "remote_reclustering_v1":
            if ArgumentEditing.hasHelpFlag(forwarded) {
                try Tooling.runPath(
                    try PythonRuntime.resolve(),
                    [try script("evaluate-session-local-remote-speaker-reclustering-feasibility-v1.py").path, "--help"]
                )
                return
            }
            guard let action = forwarded.first else {
                throw CLIError(
                    "remote-reclustering-v1 requires preflight, prepare, freeze, evaluate, replay, finalize, status, or all"
                )
            }
            guard [
                "preflight", "prepare", "freeze", "evaluate", "replay", "finalize", "status", "all",
            ].contains(action) else {
                throw CLIError("unsupported remote-reclustering-v1 action: \(action)")
            }
            _ = try Tooling.runPathAllowingExitCodes(
                try PythonRuntime.resolve(),
...
enum CorpusHelp {
    static func print() {
        ...
        usage:
          ...
          murmurmark corpus remote-identity-enrollment-v1 preflight|freeze|build|evaluate|status|replay|finalize|all
