...
- опираться на текущую точку 2026-06-29: `0.02 min` проверки для selected notes и `2.71 min`
...
- снизить `transcript_review_burden` с `2.71 min` до `<= 1.5 min` и очередь с `32` до `<= 15`
...
`speaker_state` независимо показывает почти чистый `local_only`. Следующий исполнительный блок —
