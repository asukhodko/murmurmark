...
`derived/live/` is a compatibility alias for the existing implementation files: draft transcript,
chunks, segment list, worker log, live-vs-batch comparison and final reconcile. User-facing
transcripts, notes, export and retention continue to use batch artifacts.
...
- `outputs`: links to sidecar chunks, draft, worker log, comparison and reports.
