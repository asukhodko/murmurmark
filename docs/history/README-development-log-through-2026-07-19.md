# MurmurMark Development Log Through 2026-07-19

Historical snapshot of the former project README. It preserves implementation notes, experiment
metrics and commands that were current at different stages of development. It is not the current
user guide or planning authority.

Use the repository root `README.md` for current commands and
`docs/roadmap/murmurmark-cli-roadmap.plan.yaml` for current statuses and dependencies.
...
Authoritative operating point, 2026-07-19:
...
The next development path closes finite authoritative-transcript risks before reopening broader
audio or live promotion work:

```mermaid
flowchart LR
    A["Local Recall Closure<br/>13 rows / 48.073s"] --> B["Chronology Closure<br/>14 rows / 62.690s"]
    B --> C["Operational Rebaseline<br/>profile, verdict, notes, export"]
    C --> D["Echo Promotion Readiness<br/>prevent new remote leak"]
    D --> E["Evidence Notes and Export"]
    E --> F["Release-quality CLI"]

    L["Near-realtime shadow"] -. "separate evidence branch" .-> C
    H["Heavy local validators"] -. "later, corpus-gated" .-> D
```

Every transition is conditional. A candidate remains isolated until frozen-input, per-session and
corpus-wide no-regression gates pass. A reproducible `DO_NOT_PROMOTE` is a completed result: the
authoritative profile stays selected and the next hypothesis uses a different evidence contract.
