# Goal History Through 2026-07-19

Historical snapshot of completed and formerly recommended goals. It is not the current goal
authority. Use `docs/project/current-goal.md` for the executable goal and the active OpsKarta plan
for statuses and dependencies.
...
## Route After The Current Goal

```mermaid
flowchart TD
    A["Now: Local Recall Closure<br/>13 rows / 48.073s"] --> G1{"Corpus PROMOTE?"}
    G1 -- "yes" --> B["Select isolated local-recall profile"]
    G1 -- "no" --> B0["Keep residual_me_evidence_v1<br/>record evidence ceiling"]
    B --> C["Chronology Closure<br/>14 rows / 62.690s"]
    B0 --> C
    C --> G2{"Lossless order gates pass?"}
    G2 --> D["Operational Rebaseline<br/>verdict, notes, review burden, export"]
    D --> E["Audio root-cause work<br/>Echo promotion readiness"]
```

The current goal does not have to promote to unblock the route. A deterministic `DO_NOT_PROMOTE`
closes the hypothesis and preserves `residual_me_evidence_v1`; chronology then uses its own lossless
contract. The operational rebaseline measures the actual user benefit before choosing between
further transcript repair and audio-layer suppression.
