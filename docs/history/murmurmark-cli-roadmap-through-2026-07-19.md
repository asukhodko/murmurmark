# MurmurMark CLI Roadmap Through 2026-07-19

Historical detailed roadmap. It preserves completed experiment metrics and former planning views.
It is not the current status authority. Use `docs/roadmap/murmurmark-cli-roadmap.plan.yaml` and the
current readable roadmap instead.
...
Residual Audio Evidence Arbitration v1 is the latest completed quality goal. It gives all `66`
audio-review rows stable, SHA-bound outcomes, but closes only `1` row / `0.640s`; the reproducible
`DO_NOT_PROMOTE` keeps `residual_me_evidence_v1` authoritative and identifies the evidence ceiling.

Fast Authoritative Handoff v1 remains the completed operational handoff goal. Its three-session machine
...
```mermaid
flowchart TB
    foundation["Stable CLI foundation<br/>capture, batch ASR, outcome, review, export"]
    promoted["Promoted batch profile<br/>residual_me_evidence_v1"]
    audioDone["Audio Evidence Arbitration<br/>DO_NOT_PROMOTE, evidence ceiling known"]
    local["1. Local Recall Closure<br/>13 rows / 48.073s"]
    chronology["2. Chronology Closure<br/>14 rows / 62.690s"]
    rebaseline["3. Operational Rebaseline<br/>verdict, burden, notes, export"]
    echo["4. Audio Root Cause<br/>Echo candidate promotion readiness"]
    product["5. Product Output Hardening<br/>stable notes/export contracts"]
    release["6. Release-quality local CLI"]

    foundation --> promoted --> audioDone --> local --> chronology --> rebaseline --> echo --> product --> release

    live["Near-realtime shadow"] -. "evidence only" .-> rebaseline
    corpus["Frozen corpus and gates"] --> local
    corpus --> chronology
    corpus --> echo
    heavy["Heavy local ASR / diarization"] -. "later validator" .-> echo
    llm["Evidence-guarded LLM synthesis"] -. "later" .-> product
    ui["Optional UI"] -. "tail" .-> release
```

The main path is deliberately serial where contracts can conflict. Local-recall insertion and
chronology repair are separate because a single heuristic must not both add words and reorder them.
Live, heavy-model and UI work are side branches and cannot block the useful CLI.
...
3. **Residual Chronology Closure v1.** Treat the remaining `14` order rows / `62.690s` under a
   separate lossless boundary contract. It starts after the local-recall hypothesis is closed,
   whether that result is promotion or a measured evidence limit.
4. **Residual Operational Rebaseline v1.** Recompute selected profile, notes, verdict, review burden,
   readiness and guarded export after the two residual contracts; unresolved content remains explicit.
...
6. **ASR-positive Echo promotion readiness.** Expand `coverage_v2_remote_gate_local_fir` validation
   ...
   testing.
7. **Target-Me evidence follow-up.** Keep integrating `resemblyzer_dvector_v0` with review-lane
