...
```
Speaker-aware memory and exact-text notes remain optional derivatives. Transcript Perfection Corpus keeps 21/21 frozen sources explicit and never collapses unlike quality dimensions into one score.
...
murmurmark corpus remote-identity-shadow-v1 status && murmurmark corpus remote-identity-shadow-v1 replay && murmurmark corpus remote-identity-shadow-errors-v1 status && murmurmark corpus remote-identity-shadow-errors-v1 replay
```
...
-> Remote Speaker Shadow Error Decomposition v1 (done: ADVANCE_INTERVAL_PURIFICATION) -> Bounded Remote Speaker Interval Purification v1 (current)
```
...
The independently trained ECAPA candidate passed every fixed one-shot hard-v4 gate: B-cubed F1 `0.948042`, pairwise precision `1.0`, known-speaker recall `0.947368`, zero open-set false attribution and exact 154/154 word conservation. Its frozen real-session shadow then recovered 156/851 words and 211.100/598.240 seconds, but failed the fixed 20% word gate and reached only `0.878788` precision on the available coarse independent machine reference. Error decomposition assigned 93/214 failures and 201.274s to the only material dominant axis, interval purification; production remains Coverage v3.
...
- [Stronger Identity Backend Qualification](docs/contracts/stronger-remote-speaker-identity-backend-qualification-v1.md), [ECAPA real-session shadow](docs/contracts/ecapa-remote-speaker-shadow-qualification-v1.md), [shadow error decomposition](docs/contracts/remote-speaker-shadow-error-decomposition-v1.md), its [runbook](docs/runbooks/remote-speaker-shadow-error-decomposition-v1.md), and [Speaker-Resolved Default](docs/contracts/speaker-resolved-transcript-default-v1.md)
